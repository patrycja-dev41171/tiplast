export const useCarts = () => {
  ...
  `)
    ...
    const packProduct = async (productId, quantity, parcels, sourceItems) => {
    ...
    `)
        ...
        for (const item of standalone) {
            await packProduct(item.product_id, item.quantity, parcels)
        }

        for (const groupId in grouped) {
            const itemsInGroup = grouped[groupId]

            const totalQty = itemsInGroup.reduce(
                (sum, i) => sum + i.quantity,
                0
            )

            const productId = itemsInGroup[0].product_id

            await packProduct(productId, totalQty, parcels)
        }
