...
const props = defineProps({
    ...
    statuses: { type: Array, default: () => [{ id: 'pending', label: 'Oczekuje na płatność' }, { id: 'cod', label: 'Przy odbiorze' }, { id: 'paid', label: 'Opłacone' }, { id: 'paid_cod', label: 'Opłacone przy odbiorze' }, { id: 'cancelled', label: 'Anulowane' }, { id: 'partial_refund_issued', label: 'Zlecono częściowy zwrot kosztów' }, { id: 'full_refund_issued', label: 'Zlecono całkowity zwrot kosztów' }] },
...
async function save() {
    ...
    try {
        await props.savePaymentStatus({ orderId: props.orderId, status: selectedStatus.value });
